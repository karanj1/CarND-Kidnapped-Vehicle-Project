...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
	...
	num_particles = 100;  //for debugging set it to 1 and uncomment all debugging flags (//std::cout)
    ...
    for (int i = 0; i < num_particles; ++i) {
      ...
      //std::cout<< "Initial Particle:"<<particles[i].x <<"," <<p.y <<","<< p.theta <<std::endl;
    }
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
  ...
  //To stop iteration after one looping of localization.  slelct num_particles = 1 in 'init' function
  /*if (is_initialized) {
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
    ...
    for (int i = 0; i < num_particles; ++i) {  
      ...
      for (unsigned int q = 0; q < observations.size(); q++) {
        ...
        //std::cout<<"Obs(x,y)"<<"("<<observations[q].x <<","<<observations[q].y<< ")";
		    //std::cout<<"-->TObs(x,y)"<<"("<<t_x <<","<<t_y<< ")"<<std::endl;
      ...
      //std::cout << "dataAssociation Done!!" << std::endl;

      // reinit weight
      particles[i].weight = 1.0;  

      //std::cout<<"---------------------Weight Calculations------------------------"<<std::endl;
      for (unsigned int j = 0; j < transformed_os.size(); j++) {
        ...
        for (unsigned int k = 0; k < predictions.size(); k++) {
          ...
          if (predictions[k].id == associated_prediction) {
            ...
            //std::cout <<"predictions[k].id : " <<predictions[k].id <<", pr_x:" <<pr_x <<", pr_y:"  << pr_y << std::endl;
          }
        }  
        ...
        //std::cout << obs_w << "s_x : " << s_x << ", sy: " << s_y << " : " << std::endl;
        //std::cout<<"dx:"<<pr_x-o_x<<";"<<"dy:"<<pr_y-o_y<<std::endl;
        //std::cout<<"TempProb: "<<obs_w<<std::endl;
